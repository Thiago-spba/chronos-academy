...
import { AlertTriangle, CheckCircle2, Eye, Pencil, Plus, Shuffle, Trash2 } from "lucide-react";
import MaterialEstudo from "./MaterialEstudo";
import { opcoesIcones, pendenciasMaterial, visualDoMaterial } from "../utils/temasMaterial";

const GRUPOS = [
  { id: "tecnologia", rotulo: "Tecnologia e programação" },
  { id: "matematica", rotulo: "Matemática" },
  { id: "historia", rotulo: "História" },
  { id: "geral", rotulo: "Carreira e geral" },
];
...
export default function RevisaoMaterial({ material, onChange, onRemover, onTrocarVisual, disciplina, disabled, inputClass }) {
  const [previa, setPrevia] = useState(false);
  if (!material) return null;

  const pendentes = pendenciasMaterial(material);
  const visual = visualDoMaterial(material);
  const icones = opcoesIcones();
  const secoes = material.secoes || [];
  const termos = material.termos || [];
  const partes = Math.max(1, ...secoes.map((s) => s.parte || 1));
  ...
  const conferirTudo = () =>
    onChange({
      ...
      secoes: secoes.map((s) => ({ ...s, complemento: false })),
      termos: termos.map((t) => ({ ...t, complemento: false })),
    });
  ...
  const selo = (aoConferir) => (
    <div className="flex items-center justify-between gap-2 text-[11px] font-bold text-amber-700 dark:text-amber-400">
      ...
      <button type="button" disabled={disabled} onClick={aoConferir} className="flex items-center gap-1 px-2 py-1 rounded-lg bg-amber-600 text-white hover:bg-amber-700 disabled:opacity-50">
        <CheckCircle2 className="w-3.5 h-3.5" /> Conferido
      </button>
    </div>
  );

  // Quantas secoes vieram de cada material (para ver se ficou equilibrado).
  const porParte = Array.from({ length: partes }, (_, p) => secoes.filter((s) => (s.parte || 1) === p + 1).length);
    ...
    <div className="space-y-4 p-4 rounded-2xl border border-indigo-200 dark:border-indigo-900/60 bg-indigo-50/40 dark:bg-indigo-950/20">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {onTrocarVisual && (
            <button type="button" disabled={disabled} onClick={onTrocarVisual} title="Sorteia outra cor e outro layout (sem repetir as últimas aulas)" className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-white dark:bg-slate-900 border border-stone-200 dark:border-slate-700 text-stone-700 dark:text-slate-300 disabled:opacity-50">
              <Shuffle className="w-3.5 h-3.5" /> Trocar visual
            </button>
          )}
      ...
      {partes > 1 && (
        <p className="text-xs font-bold text-stone-600 dark:text-slate-400">
          Seções por material: {porParte.map((n, p) => `Material ${p + 1}: ${n}`).join(" · ")}
        </p>
      )}
      ...
      {previa ? (
        <MaterialEstudo material={material} disciplina={disciplina} />
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            <select disabled={disabled} value={visual.icone} onChange={(e) => trocarIcone(e.target.value)} title="Ilustração" className={inputClass}>
              {GRUPOS.map((g) => (
                <optgroup key={g.id} label={g.rotulo}>
                  {icones.filter((o) => o.grupo === g.id).map((o) => <option key={o.chave} value={o.chave}>{o.rotulo}</option>)}
                </optgroup>
              ))}
            </select>
          </div>
          ...
          <div className={`space-y-2 p-3 rounded-xl border ${caixa(material.contextoComplemento)}`}>
            {material.contextoComplemento && selo(() => set("contextoComplemento", false))}
            <p className="text-[10px] font-black uppercase tracking-widest text-stone-500 dark:text-slate-400">Para começar (contexto)</p>
            <textarea disabled={disabled} rows={3} value={material.contexto || ""} onChange={(e) => set("contexto", e.target.value)} placeholder="2 a 4 frases que situam o aluno" className={`${inputClass} resize-y`} />
          </div>
          ...
          {secoes.map((s, i) => (
            <div key={i} className={`space-y-2 p-3 rounded-xl border ${caixa(s.complemento)}`}>
              {s.complemento && selo(() => setItem("secoes", i, "complemento", false))}
              {partes > 1 && <p className="text-[10px] font-black uppercase text-indigo-600 dark:text-indigo-400">Material {s.parte || 1}</p>}
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-stone-500 dark:text-slate-400">Palavras-chave (escreva igual ao texto para aparecer sublinhada)</p>
          {termos.map((t, i) => (
            <div key={i} className={`space-y-2 p-3 rounded-xl border ${caixa(t.complemento)}`}>
              {t.complemento && selo(() => setItem("termos", i, "complemento", false))}
              <div className="flex gap-2">
          ...
          <button type="button" disabled={disabled} onClick={() => set("termos", [...termos, { termo: "", definicao: "", complemento: false }])} className="flex items-center gap-1 text-xs font-bold text-indigo-700 dark:text-indigo-400 disabled:opacity-50">
